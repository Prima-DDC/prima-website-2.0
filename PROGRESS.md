...
## Phase 21 (2026-09-15): Field-expense dates + admin-configurable list columns (DONE)

Two staff-requested changes to the internal document system.

ISSUE-1 (Field Expenses Form needs a date, since staff spend multiple days on the field): the expense line items now carry a per-row `date`, so each expense is dated to the day it was incurred. Done by extending `LineItemsConfig` column types with `"date"`, adding a `date` column (first) to `expense_form.lineItems`, and requiring `date` (ISO `YYYY-MM-DD`) on each item in the schema. `OpsForm` renders a native date picker for date columns; `DocDetails` shows the date generically; the PDF template gains a Date column. Existing documents (no per-item date) still render, with an empty date cell.

ISSUE-2 (admins can configure which columns show in My Documents and Approvals): a new `ops_settings` key/value table (migration `0018`, RLS: any signed-in user reads, `manage_documents` writes) stores the per-view column selection under key `list_columns`. A pure catalog (`src/features/ops/columns.ts`) defines, per view (`my_documents`, `portal_approvals`, `admin_ops`), the available columns and their defaults; `columnsFor()` resolves the stored selection (catalog-ordered, defaults when unset, explicit empty respected). A new admin page `/admin/document-columns` (capability `manage_documents`, added to the admin nav) lets admins toggle columns per view via `ColumnSettingsForm` and the `saveDocumentColumns` action (`columns-store.ts` reads/writes the row). The three list pages (`/portal`, `/portal/approvals`, `/admin/ops`) now render their desktop-table headers and cells from the resolved column set; the document number stays as the fixed anchor column, and the "Your turn" badge and status tabs are unchanged. Available columns include Type, Submitted by, Status, Awaiting, Sign-offs, Submitted (time), and Total. Mobile summary cards are left as-is.
